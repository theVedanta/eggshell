...
export default function ApparelPage() {
  const params = useParams<{ apparelType: string }>();
  const { apparelType } = params;
  const { data: allProducts = [] } = useGetProductsBySubCategory(apparelType);
  ...
  const categoryTitle =
    apparelType?.charAt(0).toUpperCase() + apparelType?.slice(1) || "Apparel";
