...
export default function BrandPage({ params }: BrandPageProps) {
  ...
  const priceRange = brandProducts.length > 0 ? {
    min: Math.min(...brandProducts.map(p => p.price)),
    max: Math.max(...brandProducts.map(p => p.originalPrice || p.price))
  } : { min: 0, max: 0 };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-6">
        ...
        <div>
          <h1 className="text-4xl font-bold heading-gradient mb-4">{brand.name}</h1>
      ...
      {featuredProducts.length > 0 && (
        <section>
          <div className="mb-6">
            <h2 className="text-2xl font-bold mb-2">Featured from {brand.name}</h2>
      ...
      {availableCategories.length > 1 && (
        <section className="bg-muted/50 rounded-xl p-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold mb-2">Shop {brand.name} by Category</h2>
            <p className="text-muted-foreground">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {availableCategories.map(categoryId => {
              const category = categories.find(c => c.id === categoryId);
              const categoryProducts = brandProducts.filter(p => p.category === categoryId);
